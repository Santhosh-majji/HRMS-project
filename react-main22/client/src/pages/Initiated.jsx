...
const Initiated = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/initiatedData')
      .then(response => {
      ...
      .catch(error => {
        console.error('Error fetching initiated data:', error);
        setLoading(false);
        setError('Error fetching data. Please try again.'); // Set error state
      });
  }, []);

  const totalPages = Math.ceil(initiatedData.length / itemsPerPage);
  ...
  const currentItems = initiatedData.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const handleCancelSearch = () => {
    ...
    setInitiatedData(originalData); // Restore original data
  };

  return (
    <Box>
      <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography variant="h5">Onboarding Initiated</Typography>
