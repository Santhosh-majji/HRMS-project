...
const Cancelled = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/cancelledData')
      .then(response => {
  ...
  const currentItems = cancelledData.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const handleCancelSearch = () => {
    ...
    setCancelledData(originalData); // Restore original data
  };
 
  return (
    <Box>
      <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography variant="h5">Onboarding Cancelled</Typography>
